...
/// <summary>
/// Shared push/diff skeleton -- port of jellyfin-plugin-mdblist's
/// SyncPayloadBuilder.cs (itself sync_payload.py). Used by
...
public class SyncPayloadBuilder
{
    ...
    public SyncPayloadBuilder(MDBListApiClient apiClient, SyncStateStore stateStore, ILogManager logManager)
    {
        ...
        bool allowRemovals = false)
    {
        ...
        if (toRemove.Count > 0)
        {
            ...
            if (!allowRemovals)
            {
                ...
                _logger.Debug(
                    "MDBList Sync: {0} removal skipped ({1} items) - this trigger doesn't allow removals",
                    category,
                    toRemove.Count);
            }
            else if (currentItems.Count == 0 && known.Count > threshold)
            {
                ...
                skippedRemove = toRemove.Count;
                _logger.Warn(
                    "MDBList Sync: {0} removal skipped - current-items read is empty while {1} known items are on file "
                        + "(threshold {2}); treating as an unreliable read rather than a real removal",
                    category,
                    known.Count,
                    threshold);
            }
            else if (toRemove.Count > threshold)
            {
                ...
                skippedRemove = toRemove.Count;
                _logger.Warn(
                    "MDBList Sync: {0} removal skipped - {1} of {2} known items would be removed "
                        + "(threshold {3}); local library may be incomplete",
                    category,
                    toRemove.Count,
                    known.Count,
                    threshold);
