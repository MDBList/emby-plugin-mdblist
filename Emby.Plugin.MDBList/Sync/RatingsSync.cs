...
/// <summary>
/// Ratings two-way sync -- port of jellyfin-plugin-mdblist's RatingsSync.cs
/// (itself ratings_sync.py).
///
/// No local "rated at" timestamp exists in Emby's UserItemData any more
/// than it did in Jellyfin/Kodi, so this is not true last-write-wins: push
/// always runs before pull (see SyncOrchestrator), so on the first sync any
/// conflicting item is already resolved local-wins by the time pull
/// happens. After that, pull only ever applies items that changed remotely
...
public class RatingsSync
{
    ...
    /// <summary>
    /// Pulls remote rating changes into Emby.
    /// </summary>
    /// <param name="userId">The Emby user.</param>
    /// <param name="accessToken">A valid MDBList access token.</param>
