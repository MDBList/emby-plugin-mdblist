...
public sealed class SyncOrchestrator : IDisposable
{
    ...
    /// Safe to call from multiple trigger points; overlapping calls (for
    /// this user or any other) are skipped rather than queued.
    /// </summary>
    /// <param name="userId">The linked Emby user to sync.</param>
    ...
    public async Task<bool> RunAllLinkedUsersAsync(bool allowRemovals, CancellationToken cancellationToken)
    {
        ...
        {
            var user = _userManager.GetUserById(config.EmbyUserId);
            if (user is null)
            {
                continue;
            }
    ...
    private async Task<bool> RunForUserAsync(User user, UserSyncConfig config, bool allowRemovals, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (!hasLocalMedia && await AnyCategoryHasKnownItemsAsync(user.Id, cancellationToken).ConfigureAwait(false))
            {
                ...
                await _stateStore.SetLastRunSummaryAsync(user.Id, "sync skipped - local library looks empty", cancellationToken).ConfigureAwait(false);
                return false;
            }
            ...
            {
                _logger.Info("MDBList Sync: library scan in progress -- this run will not push removals");
            }
    ...
    /// last check.
    /// </summary>
    /// <param name="userId">The linked Emby user to check.</param>
    /// <param name="cancellationToken">The cancellation token.</param>
